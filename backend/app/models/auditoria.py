...
class MovimientoInventario(Base):
    """Movimientos de inventario (entrada, salida, ajuste, producción, venta)"""
    ...
    producto_materia_prima_id = Column(
        Integer, ForeignKey("productos_materia_prima.id"), nullable=True
    )
    lote_materia_prima_id = Column(
        Integer, ForeignKey("lotes_materia_prima.id"), nullable=True
    )
    ...
    referencia = Column(String(100), nullable=True)  # número de venta, orden, etc.
    fecha = Column(DateTime, default=datetime.utcnow, index=True)
    realizado_por_id = Column(Integer, ForeignKey("usuarios.id"), nullable=True)

    producto_materia_prima = relationship(
        "ProductoMateriaPrima", foreign_keys=[producto_materia_prima_id]
    )
